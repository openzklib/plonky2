//! A STARK for the ethereum RLP-encoding of recursive structured data types (i.e. lists and strings)
//! using two memories and a call stack
...
impl StarkConfiguration for RlpStark {
    #[inline]
    fn columns(&self) -> usize {
        layout::RlpRow::<u8>::SIZE
    }
...
impl<F, C, COM> Stark<F, C, COM> for RlpStark
where
    F: Copy,
    ...
    COM: Arithmetic<F>,
{
    ...
    fn eval(
        ...
        layout::RlpRow::read_eval(&mut curr, &mut next, public_inputs, &mut compiler);
    }
}
