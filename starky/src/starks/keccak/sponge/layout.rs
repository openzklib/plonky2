...
pub struct Keccak256SpongeRow<T: Copy> {
    ...
    pub invoke_permutation_filter: T,

    /// a LUT used to range-check bytes
    pub u8_lookup: T,
    pub u8_lookup_permuted: T,

    /// a LUT used ot range-check 7-bit numbers
    pub u7_lookup: T,
    pub u7_lookup_permuted: T,
    ...
    pub input_block_encoded: [T; KECCAK_RATE_U32S],

    /// columns for constructing the LUT
    pub u8_lookup_bits: [T; 8],
}
...
pub const fn u8_lut_col() -> usize {
    invoke_permutation_filter_col() + 1
}

pub const fn u8_lut_permuted_col() -> usize {
    u8_lut_col() + 1
}

pub const fn u7_lut_col() -> usize {
    u8_lut_permuted_col() + 1
}

pub const fn u7_lut_permuted_col() -> usize {
    u7_lut_col() + 1
}

pub const fn block_idx_bytes_start_col() -> usize {
    u7_lut_permuted_col() + 1
}

pub const fn block_idx_bytes_permuted_start_col() -> usize {
    block_idx_bytes_start_col() + 2
}

pub const fn hash_idx_bytes_start_col() -> usize {
    block_idx_bytes_permuted_start_col() + 2
}

pub const fn hash_idx_bytes_permuted_start_col() -> usize {
