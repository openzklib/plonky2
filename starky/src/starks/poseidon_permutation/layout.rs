...
use plonky2::hash::poseidon::{N_ROUNDS, HALF_N_FULL_ROUNDS, N_PARTIAL_ROUNDS, ALL_ROUND_CONSTANTS, Poseidon};

use crate::{starks::xor::Bits, gate::{Read, Gate}, stark::StandardConstraint, ir::{Arithmetic, Assertions, Add, Mul, Constant, Constraint}};
	...
	COM: Arithmetic<T> + StandardConstraint<T> + Constant<F, T>
{
    ...
    fn eval(curr: &Self, next: &Self, _: &[T], compiler: &mut COM) {
		...
		// ROUND_CONSTANTS
		let all_round_constants = ALL_ROUND_CONSTANTS.iter().map(|c| compiler.constant(&F::from_canonical_u64(*c))).collect::<Vec<_>>();

		// MDS_MATRIX_CIRC
		let mds_matrix_circ = <F as Poseidon>::MDS_MATRIX_CIRC.iter().map(|c| compiler.constant(&F::from_canonical_u64(*c))).collect::<Vec<_>>();

		// MDS_MATRIX_DIAG
		let mds_matrix_diag = <F as Poseidon>::MDS_MATRIX_DIAG.iter().map(|c| compiler.constant(&F::from_canonical_u64(*c))).collect::<Vec<_>>();
		...
		for (curr_round_const, this_round_constants) in curr.round_constants.iter().zip(this_round_constants) {
			compiler.assert_eq(curr_round_const, &this_round_constants);
		}

		// add round constants
		// degree 1
		let state = curr.state.iter().zip(curr.round_constants).map(|(state, constant)| compiler.add(state, &constant)).collect_vec();
