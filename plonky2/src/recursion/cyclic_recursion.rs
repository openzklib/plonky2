...
impl<F: RichField + Extendable<D>, const D: usize> CircuitBuilder<F, D> {
        ...
        C::Hasher: AlgebraicHasher<F>,
    {
        ...
        self.conditionally_verify_proof::<C>(
            // can we use this w/ other as base case padded to be same size as inductive case?
            condition,
...
mod tests {
        ...
        C::Hasher: AlgebraicHasher<F>,
    {
        ...
        println!(
            "There are {:?} gates before 0th recursion",
            builder.num_gates()
        ); // 0 gates
        let data = builder.build::<C>();
        println!("The common data after building 1 times \n{:?}", data.common);
        let data = add_recursion_layer(data);
        println!("The common data after building 2 times \n{:?}", data.common);

        let data = add_recursion_layer(data);
        println!("The common data after building 3 times \n{:?}", data.common);

        let data = add_recursion_layer(data);
        println!("The common data after building 4 times \n{:?}", data.common);

        let data = add_recursion_layer(data);

        // Now this time we do something different before building, adding some `NoopGate`s
        let config = CircuitConfig::standard_recursion_config();
        ...
        // Is this what sets the size?
        // log2 = 13 has gates 3551 before rounding
        // log2 = 12 has gates 3551 before rounding
        // log2 = 11 has gates 3551 before rounding
        println!("There are {:?} gates before rounding", builder.num_gates()); // 3844 gates = fixed point
        while builder.num_gates() < 1 << log2 {
    ...
    // Generates `CommonCircuitData` usable for recursion.
    fn common_data_for_arity_2_circuit_recursion<F, C, const D: usize>(
        degree: usize,
    ) -> CommonCircuitData<F, D>
    where
        C::Hasher: AlgebraicHasher<F>,
    ...
    /// Generate a circuit with desired degree from `base_circuit_data`.
    ...
    >(
        ...
        let num_noop_gate =
            (1 << degree_bits) - ceil_div_usize(common_data.num_public_inputs, 8) - 2;
    ...
    /// Increase the "recursion degree" of circuit data by a `CircuitBuilder`
    /// that checks proofs of this `CircuitData`.
    fn add_recursion_layer<F, C, const D: usize>(data: CircuitData<F, C, D>) -> CircuitData<F, C, D>
    where
        F: RichField + Extendable<D>,
        C: GenericConfig<D, F = F>,
        C::Hasher: AlgebraicHasher<F>,
    {
        ...
        let verifier_data =
            builder.add_virtual_verifier_data(data.common.config.fri_config.cap_height);
        builder.verify_proof::<C>(&proof, &verifier_data, &data.common);
        println!("Produced a builder with {:?} gates", builder.num_gates());
        builder.build()
    }
    ...
    fn test_cyclic_recursion() -> Result<()> {
        ...
        // Changing log2 from 12 results in CommonCircuitData being different
        // log2 = 13 gave degree_bits = 14
        // log2 = 12 gave degree_bits = 13
        // log2 = 11 gave degree_bits = 12
        let mut common_data = common_data_for_recursion::<F, C, D>(12); // How to find this fixed point for 2 verifiers?
        println!("Common data is {common_data:?}");
        ...
        let counter = builder.add_virtual_public_input(); // const. cost

        // must register PIs for base case before this
        ...
        // Base case proof
        // Do base case as verifier
        // let base_case_proof_with_pis = builder.add_virtual_proof_with_pis::<C>(&common_data);
        // let base_case_verifier_data = builder.add_ver
        // let base_case_pis = &base_case_proof_with_pis.public_inputs; // needs to move up
        // let neg_condition = builder.not(condition);
        // builder.conditionally_verify_proof_or_dummy(neg_condition, &base_case_proof_with_pis, inner_verifier_data, inner_common_data); // verifier data will be const.
        // different base case common data? Changing common data must also change 299 (up 5)

        // Unpack inner proof's public inputs.
        let inner_cyclic_proof_with_pis = builder.add_virtual_proof_with_pis::<C>(&common_data); // either base case or inductive case pf
        ...
        let new_counter = builder.mul_add(condition.target, inner_cyclic_counter, one); // computation in inductive case only, not base, but both pay
        builder.connect(counter, new_counter);

        builder.conditionally_verify_cyclic_proof_or_dummy::<C>(
            // verify is part of inductive only, but always pay
            condition, // branching condition, must represent both sides of branch
            &inner_cyclic_proof_with_pis,
            &common_data, // would more generally be base case verification data
        )?;
